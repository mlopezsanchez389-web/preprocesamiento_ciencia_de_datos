...
def transformar_reducir_vih():
    ...
    def transformar_cv_exp10_y_escalar(serie: pd.Series) -> pd.Series:
        ...
        # La serie ya está en log10(copias/mL)
        minimo = 0.0
        maximo = 8.0
    ...
    columna_objetivo = "VIH_TARDIO_FLAG"
    ...
    if "SEGURIDAD_SOC" in df.columns and "SEGURIDAD_SOCIAL" not in df.columns:
        df = df.rename(columns={"SEGURIDAD_SOC": "SEGURIDAD_SOCIAL"})
    ...
    if "SEGURIDAD_SOCIAL" in df.columns:
        df["SEGURIDAD_SOCIAL"] = mapear_seguridad_social_binario(df["SEGURIDAD_SOCIAL"])
    ...
    print("##################################################")
    print("TRANSFORMACION Y REDUCCION VIH_RED_COMPLETA")
    print("##################################################\n")
