...
def transformar_reducir_consultas_hospital():
    ...
    if "CODIGO" in df.columns:
        df["CODIGO_FREQ"] = frecuencia_absoluta_entera(df["CODIGO"])
        columnas_creadas.append("CODIGO_FREQ")
        
    if "ESPECIALIDAD" in df.columns:
        df["ESPECIALIDAD_FREQ"] = frecuencia_absoluta_entera(df["ESPECIALIDAD"])
        columnas_creadas.append("ESPECIALIDAD_FREQ")
    
    df, eliminadas = eliminar_columnas(df, ["PROCEDENCIA"])
    columnas_eliminadas.extend(eliminadas)
    
    columnas_excluir_redundancia = ["ID_TEMP"]
    ...
    titulo("Transformacion y reduccion - CONSULTAS HGM")
