...
from code_python.herramientas.reporte_consola import titulo, seccion
...
def preparar_dataframe_para_comparacion(df_clean: pd.DataFrame, comparaciones: list) -> pd.DataFrame:
    ...
    for config in comparaciones:
        ...
        if tipo == "string":
            ...
            raise ValueError(f"Tipo de comparacion no soportado: {tipo}")
...
def detectar_duplicados(
    ...
    for comp_i in componentes:
        ...
        tamanos_componentes.append(len(ids))
        sobrantes += (len(ids) - 1)

    registros_unicos = len(df_clean) - sobrantes
    distribucion_tamanos_componentes = (
        pd.Series(tamanos_componentes).value_counts().sort_index().to_dict()
        if len(tamanos_componentes) > 0
        else {}
    )
    ...
    return {
        ...
        "tamanos_componentes": tamanos_componentes,
        "distribucion_tamanos_componentes": distribucion_tamanos_componentes,
        "links": links,
...
def mostrar_resultados(
    ...
    res: dict,
    comparaciones: list,
    columnas_blocking: list | None = None,
    umbral: float | None = None,
    cantidad_muestra: int = 5
) -> None:
    titulo("Deteccion de duplicados")

    print("Total de registros:", res["total_registros"])
    print("Pares candidatos identificados:", res["pares_candidatos"])
    porcentaje_pares_match = (
        round(res["pares_match"] * 100 / res["pares_candidatos"], 4)
        if res["pares_candidatos"] > 0
        else 0
    )
    porcentaje_sobrantes = (
        round(res["sobrantes"] * 100 / res["total_registros"], 4)
        if res["total_registros"] > 0
        else 0
    )
    seccion("Resumen interpretable")
    print("Columnas de blocking:", columnas_blocking)
    print("Umbral de coincidencia:", umbral)
    print(f"Pares coincidentes sobre candidatos: {porcentaje_pares_match}%")
    print(f"Registros eliminables por duplicidad: {porcentaje_sobrantes}%")
    print("Componentes/grupos de duplicados:", res["total_componentes"])
    print("Distribucion de tamanos de grupo:", res["distribucion_tamanos_componentes"])
    print("Regla de conservacion: se conserva el ID minimo de cada grupo conectado.")
    print("Comparaciones usadas:")
    for config in comparaciones:
        print(
            "- columna={columna} | tipo={tipo} | label={label}".format(
                columna=config.get("columna"),
                tipo=config.get("tipo"),
                label=config.get("label", config.get("columna", "") + "_ok")
            )
        )
    print("Pares coincidentes:", res["pares_match"])
    print("Registros sobrantes o duplicados:", res["sobrantes"])
    print("Registros unicos:", res["registros_unicos"])
...
def ejecutar_deduplicacion(
    ...
    mostrar_resultados(
        ...
        res,
        comparaciones,
        columnas_blocking=columnas_blocking,
        umbral=umbral,
        cantidad_muestra=5
    )
