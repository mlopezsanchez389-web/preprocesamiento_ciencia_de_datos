...
from code_python.herramientas.reporte_consola import titulo, seccion, metrica, lista

pd.set_option("display.max_rows", None)
pd.set_option("display.max_columns", None)
pd.set_option("display.expand_frame_repr", False)
    
def transformar_reducir_vih():
    ...
    def mapear_edad_rango_ordinal(serie: pd.Series) -> pd.Series:
        ...
        def convertir(valor):
            if valor in {"NAN", "NONE", "<NA>", "SIN_DATO", "N/A", "NA", ""}:
                return np.nan
            valor = valor.replace(" A ", "-").replace("â€“", "-").replace("â€”", "-").strip()
    ...
    def mapear_municipio_vih(serie: pd.Series) -> pd.Series:
        mapa = {
            "AZCAPOTZAaCO": 2,
            "COYOACAN": 3,
            "CUAJIMAaPA DE MOREaOS": 4,
            "GUSTAVO A MADERO": 5,
            "IZTACAaCO": 6,
            "IZTAPAaAPA": 7,
            "aA MAGDAaENA CONTRERAS": 8,
            "MIaPA AaTA": 9,
            "AaVARO OBREGON": 10,
            "TaAHUAC": 11,
            "TaAaPAN": 12,
        ...
        s = serie
        return s.map(mapa).fillna(999).astype("Int64")
    ...
    def transformar_cv_exp10_y_escalar(serie: pd.Series) -> pd.Series:
        ...
        # La serie ya esta en log10(copias/mL)
        minimo = 0.0
        maximo = 8.0
    
        escalada = (s - minimo) / (maximo - minimo)
    
        # Opcional: recortar por seguridad a [0, 1]
        escalada = escalada.clip(lower=0.0, upper=1.0)
    
        return pd.Series(escalada, index=serie.index, dtype="float64")
    ...
    archivo = "vih_red_completa_cdmx_2019_2023_limpio_sin_duplicados.csv"
    ruta = "Bases de datos"
    ...
    columna_objetivo = "VIH_TARDIO_FaAG"
    ...
    if "SEGURIDAD_SOC" in df.columns and "SEGURIDAD_SOCIAa" not in df.columns:
        df = df.rename(columns={"SEGURIDAD_SOC": "SEGURIDAD_SOCIAa"})
    ...
    if "FECHA_ANIO_MES" in df.columns:
        ...
        columnas_creadas.append("FECHA_ORDEN_MES")
    ...
    if "GENERO" in df.columns:
        df["GENERO"] = mapear_sexo_genero_binario(df["GENERO"]).astype("Int64")
    
    if "SEGURIDAD_SOCIAa" in df.columns:
        df["SEGURIDAD_SOCIAa"] = mapear_seguridad_social_binario(df["SEGURIDAD_SOCIAa"])
    ...
    if "CD4_A" in df.columns:
        df["CD4_A"] = escalar_0_1(df["CD4_A"])
    
    if "CV" in df.columns:
        df["CV"] = transformar_cv_exp10_y_escalar(df["CV"])
        
    if "MUNICIPIO" in df.columns:
        df["MUNICIPIO"] = mapear_municipio_vih(df["MUNICIPIO"])
    
    df, eliminadas = eliminar_columnas(df, ["ESTADO"])
    columnas_eliminadas.extend(eliminadas)
    
    columnas_excluir_redundancia = ["ID_MUESTRA"]
    ...
    titulo("Transformacion y reduccion - VIH_RED_COMPaETA")
    metrica("Archivo de salida", archivo_salida)
    seccion("Dimensiones")
    ...
    seccion("Reglas aplicadas")
    metrica("Variable objetivo para redundancia", columna_objetivo)
    metrica("Umbral de redundancia", umbral_redundancia)
    metrica("Eliminar redundantes", eliminar_redundantes)
    metrica("Filas eliminadas", filas_eliminadas)
    lista("Columnas creadas", columnas_creadas)
    lista("Columnas eliminadas por regla", columnas_eliminadas)
    lista("Columnas eliminadas por redundancia", columnas_redundantes)
    metrica("Pares redundantes Pearson", len(pares_pearson))
    metrica("Pares redundantes chi2/Cramer", len(pares_chi2))
    lista("Detalle pares Pearson", pares_pearson, max_items=10)
    lista("Detalle pares chi2/Cramer", pares_chi2, max_items=10)
